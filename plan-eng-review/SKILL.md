...
### 3. Test review
Make a diagram of all new UX, new data flow, new codepaths, and new branching if statements or outcomes. For each, note what is new about the features discussed in this branch and plan. Then, for each new item in the diagram, make sure there is a C# test (NUnit / xUnit / MSTest).
...
- {Form/Panel name} — {what to test and why}

## Key Device Operations to Verify
- {operation description} on {device} (e.g. "Stage home sequence on X axis")

## Edge Cases
- {edge case} for {component or device}

## Critical Paths
- {end-to-end flow that must work, e.g. "Camera-triggered stage move with position verification"}

## Hardware Required
- {list of devices needed to run these tests}
```

This file is consumed by `/qa` and `/qa-only` as primary test input. Include only information that helps a tester know **what to test and with which hardware** — not implementation details.

### 4. Performance & Real-Time Review
Evaluate:
* Control loop timing. For every new device polling loop: required cycle time, whether Thread.Sleep vs Timer is appropriate, and whether jitter is acceptable for the application.
* UI thread blocking. For every device operation: is it offloaded to Task/BackgroundWorker? Is there a CancellationToken? Does the UI remain responsive?
* Buffer management. For every communication buffer: is there a size bound? What happens when it fills up?
* Camera performance. For every image acquisition path: frame rate requirement, processing pipeline latency, frame drop handling.
* Memory in long sessions. For every new data structure: what is the memory footprint after 8 hours of continuous operation?
* Slow or high-complexity code paths. Flag anything with O(n²) or worse that runs on device data.
